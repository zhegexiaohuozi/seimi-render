...
// [seimi patch] 接入层鉴权实现。
// auth_handler_ 签名是 (string, string) -> bool，上游未文档化其参数语义。
// 这里约定：第一个参数是客户端提交的 token 值（已去掉 "Bearer " 前缀），
// 第二个参数是请求路径（供 handler 需要时区分接口）。handler 返回 true=放行。
bool server::enforce_auth_(const httplib::Request& req, httplib::Response& res) {
    auth_handler h;
    {
        std::lock_guard<std::mutex> lock(mutex_);
        h = auth_handler_;
    }
    if (!h) return false;  // 未设鉴权 handler：放行
    ...
    // 2) ?token=<token>（便于无法设置 header 的客户端/浏览器场景）
    if (token.empty() && req.has_param("token")) {
        token = req.get_param_value("token");
    }

    // 恒定时间比较由调用方在 handler 内部完成（seimi 注入的 handler 用
    // constantTimeEquals），这里只负责把 token 传过去判定。
    if (!token.empty() && h(token, req.path)) return false;  // 通过

    // 未通过：返回 401。CORS 头仍给（OPTIONS 已由专门的 handler 处理 204），
    // 这里补上允许后续客户端重试。
    res.set_header("Access-Control-Allow-Origin", "*");
    res.set_header("WWW-Authenticate", "Bearer");
    res.status = 401;
    res.set_content(R"({"jsonrpc":"2.0","error":{"code":-32001,"message":"unauthorized"}})",
                    "application/json");
    return true;  // 已填 401，调用方应 return
}

void server::handle_sse(const httplib::Request& req, httplib::Response& res) {
    if (enforce_auth_(req, res)) return;  // [seimi patch] 401 已填
    ...
    // SSE stream established successfully (explicit so logger sees the real status)
    res.status = 200;
...
void server::handle_jsonrpc(const httplib::Request& req, httplib::Response& res) {
    ...
    std::shared_ptr<event_dispatcher> dispatcher;
    {
        ...
        if (disp_it == session_dispatchers_.end()) {
            ...
            // [seimi patch] 无状态工具 session 自动重建（详见 handle_post 同名 patch 注释）。
            // seimi-render 工具无状态，过期 session 自动沿用 client 的 id 重建，对 client 透明。
            if (session_dispatchers_.size() >= max_sessions_ && max_sessions_ != 0) {
                LOG_ERROR("Too many sessions for auto-recreate: ", session_id);
                res.status = 503;
                res.set_content("{\"error\":\"Too many sessions\"}", "application/json");
                return;
            }
...
void server::handle_mcp_post(const httplib::Request& req, httplib::Response& res) {
    ...
    if (!is_initialize) {
        ...
        std::lock_guard<std::mutex> lock(mutex_);
        if (session_dispatchers_.find(session_id) == session_dispatchers_.end()) {
            // [seimi patch] 无状态工具的 session 自动重建。
            // seimi-render 的工具（render_url/get_render_result）完全无状态——每次调用
            // 都是对本进程渲染 API 的独立 HTTP 请求，不依赖任何跨请求会话状态。
            // 但 MCP Streamable HTTP transport 要求请求带有效 session id。若严格校验，
            // agent（如 zcode/Claude Code）在 session 超时回收后恢复对话会收到
            // "Session not found"，必须重新 initialize，体验差且非标准。
            // 这里对过期/未知的 session 自动重建（沿用 client 提供的 id，对 client 透明），
            // 让无状态工具调用永远成功。受 max_sessions 上限保护（防止 id 洪水）。
            if (session_dispatchers_.size() >= max_sessions_ && max_sessions_ != 0) {
                res.status = 503;
                res.set_content("{\"error\":\"Too many sessions\"}", "application/json");
                return;
            ...
            auto session_dispatcher = std::make_shared<event_dispatcher>();
            session_dispatchers_[session_id] = session_dispatcher;
            // session_initialized_ 标记已初始化（避免后续 process_request 里 initialize 检查失败）
            session_initialized_[session_id] = true;
        }
    }
    ...
    if (is_initialize) {
        ...
        res.set_header("Content-Type", "application/json");
        res.status = 200; // explicit so logger sees the real status
    ...
    // Request(s) processed successfully — set status up front so both the SSE
    // and inline JSON branches below (and the logger) see the real status,
    // rather than relying on httplib's -1 -> 200 normalization.
    res.status = 200;
...
void server::handle_mcp_get(const httplib::Request& req, httplib::Response& res) {
    ...
    std::shared_ptr<event_dispatcher> dispatcher;
    {
        ...
        if (it == session_dispatchers_.end()) {
            // [seimi patch] 无状态工具 session 自动重建（详见 handle_post 同名 patch）。
            // GET（SSE 订阅）同理：过期 session 沿用 client id 重建。
            if (session_dispatchers_.size() >= max_sessions_ && max_sessions_ != 0) {
                res.status = 503;
                res.set_content("{\"error\":\"Too many sessions\"}", "application/json");
                return;
