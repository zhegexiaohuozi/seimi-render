/*
 * seimi-render 的 Readability 正文提取包装器。
 *
 * 把 Mozilla Readability.js（Apache-2.0）内联进来，并适配为可在 QWebEnginePage::runJavaScript
 * 里直接执行的形式（原始 Readability.js 用 CommonJS module.exports，浏览器/runJavaScript 上下文
 * 里 module 未定义，会自动跳过导出，Readability 作为全局构造函数可用）。
 *
 * 用法（被 RenderPool::extractReadability 注入）：
 *   (function(){ ...本文件全部内容... ; return extractReadability(); })()
 *
 * 返回 JSON 字符串：{ ok, title, content, textLength, excerpt }
 *   - content: Readability 抽取的正文 HTML（已序列化），喂给 html2md::Convert
 *   - 抽取失败（如非文章页）ok=false，调用方回退到保守策略
 *
 * 注意：Readability.parse() 会 mutate 传入的 document，必须用 document.cloneNode(true)
 * 深拷贝一份再传入，避免污染原始 DOM（原始 DOM 还要用于 html 输出和截图）。
 */
(function(){

// ====== Mozilla Readability.js 内联（Apache-2.0，https://github.com/mozilla/readability）======
// 完整保留上游算法逻辑，不做任何精简或修改，以保证正文定位准确率。
// 文件较长（~2800行），此处为原文内联。为了便于升级，整段以 BEGIN/END 标记包裹。
// [[[ READABILITY_BEGIN ]]]
/*
...
// ====== seimi 适配层 ======
// 在当前页面上跑 Readability，把正文 HTML 存入 window.__seimiReadability 供 C++ 分块读取。
// 不直接返回大字符串（QWebEnginePage::runJavaScript 回调对大返回值会截断）。
// C++ 侧先读 window.__seimiReadability.status（小 JSON，不截断），再分块读 content。
function extractReadability() {
  try {
    var docClone = document.cloneNode(true);
    docClone.querySelectorAll('script, style, noscript').forEach(function(el){ el.remove(); });
    ...
    if (!result || !result.content) {
      window.__seimiReadability = { status: 'failed', reason: 'not article-like or parse failed' };
      return JSON.stringify({ ok: false });
    }
    // 正文按 CHUNK_SIZE 切块存数组，C++ 逐块读取后拼接。
    var content = result.content;
    var CHUNK = 30000;  // 每块 30k 字符，远小于 runJavaScript 返回值上限
    var chunks = [];
    for (var i = 0; i < content.length; i += CHUNK) {
      chunks.push(content.substring(i, i + CHUNK));
    }
    window.__seimiReadability = {
      status: 'ok',
      title: result.title || '',
    ...
    return JSON.stringify({ ok: true, totalChunks: chunks.length, title: result.title || '' });
  } catch (e) {
    window.__seimiReadability = { status: 'error', reason: String(e && e.message ? e.message : e) };
    return JSON.stringify({ ok: false });
  }
}

return extractReadability();
